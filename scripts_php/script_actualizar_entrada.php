...
<body>
    ...
    <div class="container mt-3">
        ...
        <h4>Actualización de Entrada</h4>
        <?php
            $id = $_POST['id'];
            $nombre = $_POST['nombre'];
            $date = $_POST['date'];
            $entrada_nueva = date("h:i A", strtotime($_POST["entrada_nueva"]));
            date_default_timezone_set('America/Monterrey');
            $fecha_cambio = date("Y/m/d"); 
            print("$id $nombre $date $entrada_nueva ");
            ...
            if(!$conn=conectar()){
                ...
                $sql = "UPDATE pase_lista SET hora_entrada = '$entrada_nueva', fecha_registro = '$fecha_cambio' WHERE id_trabajador='$id' AND fecha_lista = '$date'";
                ...
                if ($query){
                    print(" <div class='alert alert-primary alert-dismissible fade show'>
                                ...
                                <strong>¡La hora de entrada ha sido actualizada exitosamente!</strong>
                            </div>");
                }
                else{
                    print(" <div class='alert alert-danger alert-dismissible fade show'>
                                <a href='../rh_nomina.php'><button type='button' class='btn-close' data-bs-dismiss='alert'></button></a>
                                <strong>No se pudo actualizar la hora de entrada, Error: '. $sql . ' '. mysqli_error($conn);</strong>
                            </div>");
                }
            }
            mysqli_close($conn);
